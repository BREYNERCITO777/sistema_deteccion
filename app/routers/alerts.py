...
async def list_alerts(limit: int = 50, db: AsyncIOMotorDatabase = Depends(get_db)):
    return await alert_repo.list(db, limit=limit)


@router.post("", operation_id="alerts_create")
async def create_alert(payload: AlertCreate, db: AsyncIOMotorDatabase = Depends(get_db)):
...
async def mark_read(alert_id: str, read: bool = True, db: AsyncIOMotorDatabase = Depends(get_db)):
...
@router.delete("", operation_id="alerts_delete_all")
async def delete_all_alerts(db: AsyncIOMotorDatabase = Depends(get_db)):
    deleted = await alert_repo.delete_all(db)
